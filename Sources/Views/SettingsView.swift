...
struct SettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                Button { onBack() } label: {
                    Image(systemName: "chevron.left").foregroundStyle(Color.secondary)
                }.buttonStyle(.plain)
                Text("Settings").font(.headline)
                Spacer()
            }

            Toggle("Show emoji in the menu bar", isOn: $settings.showEmojiInMenuBar)
                .font(.system(size: 13))
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 4) {
                    Text("Theme").font(.caption).foregroundStyle(Color.secondary)
                    if !model.isPro {
            ...
            HStack {
                Text(model.isPro ? "Untill Pro · active" : "Free version")
